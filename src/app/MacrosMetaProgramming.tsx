...
const animationScript = [
  { explanation: "Let's see how macros and meta programming work in Rust.", codeLineIdx: 0, state: { macro: true, call: false, proc: false } },
  { explanation: 'Define a macro with macro_rules!.', codeLineIdx: 1, state: { macro: true, call: false, proc: false } },
  { explanation: 'Call the macro.', codeLineIdx: 5, state: { macro: true, call: true, proc: false } },
  { explanation: 'Procedural macros use #[proc_macro].', codeLineIdx: 7, state: { macro: true, call: true, proc: true } },
];

function MacrosMetaProgramming3D({ state }: { state: any }) {
  return (
    <group>
      <Box position={[-4, 2, 0]} args={[2, 1, 1]}>
        <meshStandardMaterial color={'#ffd54f'} />
        <Text position={[0, 0, 0.6]} fontSize={0.22} color="#333">macro_rules!</Text>
      </Box>
      {state.call && (
        <Box position={[0, 2, 0]} args={[2, 1, 1]}>
          <meshStandardMaterial color={'#81c784'} />
          <Text position={[0, 0, 0.6]} fontSize={0.22} color="#333">say_hello!()</Text>
        </Box>
      )}
      {state.proc && (
        <Box position={[4, 2, 0]} args={[2, 1, 1]}>
          <meshStandardMaterial color={'#64b5f6'} />
          <Text position={[0, 0, 0.6]} fontSize={0.22} color="#333">#[proc_macro]</Text>
        </Box>
      )}
    </group>
  );
...
export default function MacrosMetaProgramming() {
    ...
    <>
      <div style={{ width: '100vw', height: '100vh', background: 'linear-gradient(135deg, #232526 0%, #414345 100%)' }}>
        <Canvas camera={{ position: [0, 0, 10], fov: 50 }}>
          ...
          <MacrosMetaProgramming3D state={state} />
          <OrbitControls />
        </Canvas>
