...
const codeLines = [
  ...
  '}',
  '',
  'let res1 = parse_num("42"); // Ok(42)',
  'let res2 = parse_num("oops"); // Err(ParseIntError)',
];

const animationScript = [
  { explanation: "Let's see how the ? operator propagates errors.", codeLineIdx: 0, state: {} },
  { explanation: 'We define a function that parses a string to an i32.', codeLineIdx: 1, state: { input: '42', result: 'Ok(42)' } },
  { explanation: 'The ? operator returns early if an error occurs.', codeLineIdx: 2, state: { input: 'oops', result: 'Err(ParseIntError)' } },
  { explanation: 'If parsing succeeds, we return Ok(n).', codeLineIdx: 3, state: { input: '42', result: 'Ok(42)' } },
  { explanation: 'If parsing fails, the error is propagated up.', codeLineIdx: 6, state: { input: 'oops', result: 'Err(ParseIntError)' } },
];

function Propagation3D({ state }: { state: any }) {
  return (
    <group>
      <Text position={[-3, 2, 0]} fontSize={0.3} color="#fff">parse_num(s: &str)</Text>
      <Box position={[-3, 0.5, 0]} args={[2, 1, 1]}>
        <meshStandardMaterial color="#ffd54f" />
        <Text position={[0, 0, 0.6]} fontSize={0.22} color="#333">input: {state.input ?? ''}</Text>
      </Box>
      <Box position={[3, 0.5, 0]} args={[2, 1, 1]}>
        <meshStandardMaterial color={state.result?.startsWith('Ok') ? '#81c784' : '#e57373'} />
        <Text position={[0, 0, 0.6]} fontSize={0.22} color="#333">{state.result ?? ''}</Text>
      </Box>
    </group>
  );
}
