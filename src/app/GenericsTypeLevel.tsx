...
const animationScript = [
  { explanation: "Let's see generics and type-level programming.", codeLineIdx: 0, state: { struct: true, fn: false, call: false } },
  { explanation: 'Define a generic struct.', codeLineIdx: 1, state: { struct: true, fn: false, call: false } },
  { explanation: 'Define a generic function with trait bounds.', codeLineIdx: 5, state: { struct: true, fn: true, call: false } },
  { explanation: 'Use generics with different types.', codeLineIdx: 9, state: { struct: true, fn: true, call: true } },
];

function GenericsTypeLevel3D({ state }: { state: any }) {
  return (
    <group>
      <Box position={[-4, 2, 0]} args={[2, 1, 1]}>
        <meshStandardMaterial color={'#ffd54f'} />
        <Text position={[0, 0, 0.6]} fontSize={0.22} color="#333">Point&lt;T&gt;</Text>
      </Box>
      {state.fn && (
        <Box position={[0, 2, 0]} args={[2, 1, 1]}>
          <meshStandardMaterial color={'#81c784'} />
          <Text position={[0, 0, 0.6]} fontSize={0.22} color="#333">add&lt;T&gt;</Text>
        </Box>
      )}
      {state.call && (
        <Box position={[4, 2, 0]} args={[2, 1, 1]}>
          <meshStandardMaterial color={'#64b5f6'} />
          <Text position={[0, 0, 0.6]} fontSize={0.22} color="#333">p, f</Text>
        </Box>
      )}
    </group>
  );
...
export default function GenericsTypeLevel() {
