...
const animationScript = [
  { explanation: "Let's see how Rust manages dependencies with Cargo.", codeLineIdx: 0, state: { cargo: true, deps: false, rand: false } },
  { explanation: 'Add dependencies in Cargo.toml under [dependencies].', codeLineIdx: 5, state: { cargo: true, deps: true, rand: false } },
  { explanation: 'Here, we add the rand crate.', codeLineIdx: 6, state: { cargo: true, deps: true, rand: true } },
  { explanation: 'Now we can use rand in our code.', codeLineIdx: 9, state: { cargo: true, deps: true, rand: true, use: true } },
];

function Dependency3D({ state }: { state: any }) {
  return (
    <group>
      {/* Cargo.toml */}
      ...
      {state.deps && (
        <Text position={[-1, 2, 0]} fontSize={0.18} color="#64b5f6">[dependencies]</Text>
      )}
      {/* rand crate */}
      {state.rand && (
        <Box position={[2, 2, 0]} args={[2, 1, 1]}>
          <meshStandardMaterial color={'#ffd54f'} />
          <Text position={[0, 0, 0.6]} fontSize={0.22} color="#333">rand</Text>
        </Box>
      )}
      {/* use rand in code */}
      {state.use && (
        <Text position={[2, 0.7, 0]} fontSize={0.18} color="#81c784">use rand::Rng</Text>
      )}
    </group>
  );
}
