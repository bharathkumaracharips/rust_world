...
const animationScript = [
  { explanation: "Let's see how benchmarking works in Rust.", codeLineIdx: 0, state: { cargo: true, feature: false, extern: false, bench: false } },
  { explanation: 'Add test as a dev-dependency in Cargo.toml.', codeLineIdx: 2, state: { cargo: true, feature: false, extern: false, bench: false } },
  { explanation: 'Enable the test feature and import extern crate test.', codeLineIdx: 5, state: { cargo: true, feature: true, extern: true, bench: false } },
  { explanation: 'Write a #[bench] function using test::Bencher.', codeLineIdx: 7, state: { cargo: true, feature: true, extern: true, bench: true } },
];

function Benchmarking3D({ state }: { state: any }) {
  return (
    <group>
      ...
      {state.feature && (
        <Text position={[-1, 2, 0]} fontSize={0.18} color="#ffd54f">#![feature(test)]</Text>
      )}
      {state.extern && (
        <Text position={[2, 2, 0]} fontSize={0.18} color="#81c784">extern crate test</Text>
      )}
      {state.bench && (
        <Text position={[2, 0.7, 0]} fontSize={0.18} color="#e57373">#[bench] fn bench_add</Text>
      )}
    </group>
  );
}
