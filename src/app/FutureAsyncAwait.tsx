...
const animationScript = [
  { explanation: "Let's see how async/await and futures work in Rust.", codeLineIdx: 0, state: { main: true, asyncfn: false, await: false, done: false } },
  { explanation: 'Define an async function.', codeLineIdx: 1, state: { main: true, asyncfn: true, await: false, done: false } },
  { explanation: 'Call .await to suspend until ready.', codeLineIdx: 6, state: { main: true, asyncfn: true, await: true, done: false } },
  { explanation: 'block_on runs the future to completion.', codeLineIdx: 9, state: { main: true, asyncfn: true, await: true, done: true } },
];

function FutureAsyncAwait3D({ state }: { state: any }) {
  return (
    <group>
      ...
      {state.asyncfn && (
        <Box position={[0, 2, 0]} args={[2, 1, 1]}>
          ...
          <Text position={[0, 0, 0.6]} fontSize={0.22} color="#333">async fn</Text>
        </Box>
      )}
      {state.await && (
        <Box position={[4, 2, 0]} args={[2, 1, 1]}>
          <meshStandardMaterial color={'#64b5f6'} />
          <Text position={[0, 0, 0.6]} fontSize={0.22} color="#333">.await</Text>
        </Box>
      )}
      {state.done && (
        <Text position={[0, 0.7, 0]} fontSize={0.18} color="#e57373">Done!</Text>
      )}
    </group>
  );
}
...
export default function FutureAsyncAwait() {
