...
const animationScript = [
  { explanation: "Let's see atomic operations for thread-safe increments.", codeLineIdx: 0, state: { counter: 0, threads: 0, done: false } },
  { explanation: 'Spawn 10 threads, each increments the counter.', codeLineIdx: 5, state: { counter: 0, threads: 10, done: false } },
  { explanation: 'Each thread calls fetch_add(1).', codeLineIdx: 8, state: { counter: 10, threads: 10, done: false } },
  { explanation: 'All threads join, final value is loaded.', codeLineIdx: 12, state: { counter: 10, threads: 10, done: true } },
];

function AtomicOperation3D({ state }: { state: any }) {
  return (
    <group>
      <Box position={[-4, 2, 0]} args={[2, 1, 1]}>
        <meshStandardMaterial color={'#ffd54f'} />
        <Text position={[0, 0, 0.6]} fontSize={0.22} color="#333">AtomicUsize</Text>
      </Box>
      <Text position={[-4, 0.7, 0]} fontSize={0.18} color="#81c784">{state.counter}</Text>
      {/* Threads */}
      {Array.from({ length: state.threads || 0 }).map((_, i) => (
        <Box key={i} position={[2, 2 - i * 0.5, 0]} args={[1, 0.4, 0.4]}>
          <meshStandardMaterial color={'#64b5f6'} />
          <Text position={[0, 0, 0.3]} fontSize={0.13} color="#333">T{i+1}</Text>
        </Box>
      ))}
      {/* Done */}
      {state.done && (
        <Text position={[-4, -1, 0]} fontSize={0.18} color="#e57373">Done!</Text>
      )}
    </group>
  );
}
...
export default function AtomicOperation() {
